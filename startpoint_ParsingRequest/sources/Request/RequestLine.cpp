...
RequestLine::RequestLine(const std::string &raw, const std::string &method, const std::string &url, const std::string &http) :
 requestLine(raw), methode(method), uri(url), version(http)
{
}
...
RequestLine &RequestLine::operator=(const RequestLine &ref)
{
	if (this != &ref)
	{
		this->requestLine = ref.requestLine;
		this->methode = ref.methode;
		this->uri = ref.uri;
